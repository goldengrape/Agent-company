...
## 1. 项目概述
本项目旨在开发一个高度组织化的“Agent 公司”系统。该系统建立在 **nanobot** 轻量级框架之上，支持在同一工作区内运行多个独立或协作的“Agent 公司”。系统通过模拟现实世界的科层制/官僚体系，利用“公文流”控制多个 Agent 协作完成复杂任务。每个公司实例通过标准化的文档定义人（岗位）、事（流程）和物（公文格式）。
